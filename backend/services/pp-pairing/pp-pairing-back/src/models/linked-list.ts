 ...
 */
class ItemIterator<T extends Item> {
  item: T | null;
  /**
...
export class List<T extends Item> {
   ...
   */
  static from<U extends Item>(items: Array<U | null | undefined>) {
    /** @type {List<T>} */
    const list = new this();
   ...
   * @returns {List<T>}
   */
  static of<U extends Item>(...items: Array<U | null | undefined>) {
    /** @type {List<T>} */
    const list = new this();
    return appendAll(list, items);
  }
 ...
 * @returns {TheList}
 */
function appendAll<U extends Item>(
  list: List<U>,
  items: Array<U | null | undefined> | undefined
) {
  if (!items) {
    return list;
  }
